...
- (void)testGetTransitInstructions
{
    ...
    [instance transitFrom:Apple to:StocktonSt withOptions:nil completionHandler:^(AlbequerqueResult *result) {
...
- (void)testGetTransitInstructionsWithMappingData
{
    ...
    [instance transitFrom:Apple to:StocktonSt withOptions:options completionHandler:^(AlbequerqueResult *result) {
        STAssertNotNil(result, @"Result should not be nil");
        STAssertTrue(result.legs.count > 0, @"Result should have at least 1 leg");
        for (AlbequerqueLeg *leg in result.legs) {
